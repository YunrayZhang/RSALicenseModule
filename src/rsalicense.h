...
public:
    ...
    static std::string encrypt(const unsigned char *msg,  int msgLen, const BigInteger &N,
                               const BigInteger &E);

    //RSA Encode and then use base64 to encode to make the cipher readable
    static std::string encrypt_base64ed(const unsigned char *msg,  int msgLen, const BigInteger &N,
                                        const BigInteger &E);

    //RSA decode
    static std::vector<unsigned char> decrypt(const unsigned char *cipher, int cipherLen, const BigInteger &N,
                                              const BigInteger &D);
